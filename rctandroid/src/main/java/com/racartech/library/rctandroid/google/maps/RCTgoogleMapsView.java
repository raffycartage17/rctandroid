...
public class RCTgoogleMapsView extends FrameLayout implements OnMapReadyCallback, RCTfacingDirectionListener.FacingDirectionListener {
    ...
    public RCTgoogleMapsView(@NonNull Context context, Activity the_activity, long update_interval, long fastest_interval) {
        super(context);
        init();
        activity = the_activity;
    }

    public RCTgoogleMapsView(@NonNull Context context, @Nullable AttributeSet attrs, Activity the_activity, long update_interval, long fastest_interval) {
        super(context, attrs);
        init();
        activity = the_activity;
    }

    public RCTgoogleMapsView(@NonNull Context context, @Nullable AttributeSet attrs, int defStyleAttr, Activity the_activity, long update_interval, long fastest_interval) {
    ...
    // Method to be called
    private double setCurrentDirectionsTotalDistance(double total_distance) {
    ...
    public void drawPolyline(ArrayList<LatLng> coordinates){
        PolylineOptions polyline_options = RCTgoogleMapsUtil.getDirectionRoutePolylineOptions(coordinates);
        googleMap.addPolyline(polyline_options);
    }
    public void drawPolyline(PolylineOptions polyline_options){
        googleMap.addPolyline(polyline_options);
    }

    public void drawPolyline(ArrayList<LatLng> coordinates, int color){
        PolylineOptions polyline_options = RCTgoogleMapsUtil.getDirectionRoutePolylineOptions(coordinates,color);
        googleMap.addPolyline(polyline_options);
    }



    public void drawDirectionRoutes(ArrayList<DirectionsRoute> direction_routes){
        new Thread(new Runnable() {
            @Override
            public void run() {

                PolylineOptions polyline_options = RCTgoogleMapsUtil.getDirectionRoutePolylineOptions(direction_routes);
                double driving_distance = RCTgoogleMapsUtil.getDrivingDistance_M(direction_routes);


                ArrayList<LatLng> polyline_coordinates = new ArrayList<>(polyline_options.getPoints());
                ...
                activity.runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        ...
                        CURRENT_DIRECTIONS_TOTAL_DISTANCE.set(driving_distance);
    ...
    public void drawDirectionRoutes(ArrayList<DirectionsRoute> direction_routes, int color){
        new Thread(new Runnable() {
            @Override
            public void run() {

                PolylineOptions polyline_options = RCTgoogleMapsUtil.getDirectionRoutePolylineOptions(direction_routes, color);
                double driving_distance = RCTgoogleMapsUtil.getDrivingDistance_M(direction_routes);
    ...
    public void drawDirectionRoutes_01(
            ArrayList<DirectionsRoute> directions_routes,
            ArrayList<PolylineOptions> polyline_options
    ){
        new Thread(new Runnable() {
            @Override
            public void run() {

                double driving_distance = RCTgoogleMapsUtil.getDrivingDistance_M(directions_routes);

                ArrayList<LatLng> polyline_coordinates = new ArrayList<>();

                for(int index = 0; index<polyline_options.size(); index++){
                    polyline_coordinates.addAll(polyline_options.get(index).getPoints());
                }
                ...
                activity.runOnUiThread(new Runnable() {
                    ...
                    public void run() {
                        for(int index = 0; index<polyline_options.size(); index++){
                            googleMap.addPolyline(polyline_options.get(index));
                        }
                        CURRENT_DIRECTIONS_TOTAL_DISTANCE.set(0.0);
                        CURRENT_DIRECTIONS_TOTAL_DISTANCE.set(driving_distance);
                        setCurrentDirectionsTotalDistance(CURRENT_DIRECTIONS_TOTAL_DISTANCE.get());
    ...
    public void drawDirectionRoutes(ArrayList<DirectionsRoute> direction_routes, ArrayList<Integer> polyline_colors){
        new Thread(new Runnable() {
            @Override
            public void run() {

                ArrayList<PolylineOptions> polyline_options = RCTgoogleMapsUtil.getDirectionRoutePolylineOptions(direction_routes, polyline_colors);
                double driving_distance = RCTgoogleMapsUtil.getDrivingDistance_M(direction_routes);
    ...
    public void drawDirectionRoutes(
            String api_key,
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                ...
                ArrayList<DirectionsRoute> direction_routes = RCTgoogleMapsUtil.getMultiPointDirectionRoute(
                        api_key,
                        origin,
                        destinations,
                        travel_mode,route_restriction,
                        departure_time
                );
                drawDirectionRoutes(direction_routes);
            }
        }).start();
    }
